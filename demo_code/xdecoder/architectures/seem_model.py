...
class SEEM_Model(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        openimage_switch = {
            "grounding": dec_cfg["OPENIMAGE"]["GROUNDING"].get("ENABLED", False),
        ...
        dilation_kernel = torch.ones((1, 1, dilation, dilation))
    ...
    def forward(self, batched_inputs, mode="default"):
        if self.training:
            ...
            if self.task_switch["openimage"] and self.task_switch["openimage"]["mask"]:
                ...
                losses_openimage = {
                    key.replace("grounding", "grounding_openimage"): value for key, value in losses_openimage.items()
                }
    ...
    def forward_seg(self, batched_inputs):
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.task_switch["grounding"]:
                grounding_tokens = [
                    x["grounding_query_embs"] for x in targets
                ]  # need to pad for more than one grounding token
            ...
            if self.task_switch["spatial"]:
                ...
                fp_masks = torch.stack(
                    [(x["spatial_query"]["rand_shape"].to(self.device) & False) for x in batched_inputs]
                )
                extra.update(
                    {
                        "spatial_query_pos_mask": pos_masks,
                        "spatial_query_neg_mask": neg_masks,
                        "false_positive_mask": fp_masks,
                    }
                )
        ...
        extra = {
            ...
            "class_embeddings": getattr(
                self.sem_seg_head.predictor.lang_encoder, "{}_text_embeddings".format("default")
            ),
    ...
    def evaluate_demo(self, batched_inputs):
        ...
        if "audio" in batched_inputs[0]:
            gtext = self.sem_seg_head.predictor.lang_encoder.get_text_token_embeddings(
                batched_inputs[0]["audio"], name="grounding", token=False, norm=False
            )
            token_emb = gtext["token_emb"]
            tokens = gtext["tokens"]
            query_emb = token_emb[tokens["attention_mask"].bool()]
        ...
        outputs = self.sem_seg_head.predictor(
            multi_scale_features, mask_features, target_queries=queries_grounding, extra=extra, task="demo"
        )
        ...
        for i in range(self.interactive_iter):
            outputs = self.sem_seg_head.predictor(
                multi_scale_features, mask_features, target_queries=queries_grounding, extra=extra, task="spatial"
            )
            extra.update(outputs)
            ...
            b = batched_inputs[0]
            pred_smask_all = (
                F.interpolate(pred_smask[:, :, : s[0], : s[1]], (b["height"], b["width"]), mode="bicubic")[
                    :, 0
                ].sigmoid()
                > 0.5
            )
    ...
    def evaluate(self, batched_inputs):
        ...
        box_pred_results = (
            outputs["pred_boxes"] if self.task_switch["bbox"] else [None for i in range(len(mask_pred_results))]
        )
    ...
    def evaluate_referring_image(self, batched_inputs, extra={}):
        ...
        outputs = self.sem_seg_head.predictor(
            multi_scale_features, mask_features, target_queries=queries_grounding, extra=extra, task="refimg"
        )
    ...
    def evaluate_grounding(self, batched_inputs, mode):
        ...
        processed_results = []
        for mask_pred_result, input_per_image, image_size in zip(mask_pred_results, batched_inputs, images.image_sizes):
            height = input_per_image.get("height", image_size[0])
            width = input_per_image.get("width", image_size[1])
            processed_results.append({})
    ...
    def evaluate_grounding_sptial(self, batched_inputs, mode):
        ...
        for idx, batch_per_image in enumerate(batched_inputs):
            ...
            for idx2, anno_text in enumerate(grd_texts):
                ...
                gtext = self.sem_seg_head.predictor.lang_encoder.get_text_token_embeddings(
                    [anno_text[0]], name="grounding", token=False, norm=False
                )
                token_emb = gtext["token_emb"]
                tokens = gtext["tokens"]
            ...
            if self.task_switch["grounding"]:
                ...
                if len(grd_masks) == 0:
                    ...
                    padded_masks = torch.zeros(
                        (grd_masks.shape[0], h_pad, w_pad), dtype=grd_masks.dtype, device=grd_masks.device
                    )
    ...
    def prepare_next_spaital_mask(self, outputs, batched_inputs):
        ...
        pred_masks = (
            F.interpolate(
                outputs["prev_mask"], size=gt_masks.shape[-2:], mode="bilinear", align_corners=False
            ).sigmoid()
            > 0.5
        )
        ...
        iou = (gt_masks & pred_masks).sum(list(range(1, len(fn.shape)))) / (
            (gt_masks | pred_masks).sum(dim=list(range(1, len(fn.shape)))) + 1e-8
        )
        ...
        mask_dt = (
            distance_transform((~F.pad(select_mask, pad=(1, 1, 1, 1), mode="constant", value=0)).float())[
                :, :, 1:-1, 1:-1
            ]
        ).reshape(n, -1)
